...
foreach y in `y_list' {
	...
	foreach age_group in `age_order_list' {
		...
		reghdfe ln_`y'  i.mm i.rm i.disp i.rm_disp $list_of_controls if age_category == `age_group',  absorb(StateFIPS year_group_id ) vce(cluster StateFIPS)
...
// Make table for marijuana use 
	...
	esttab  ln_`y'_4  ln_`y'_5  ln_`y'_6 ln_`y'_1 ln_`y'_3 ///
		using "output/tables/mj_full_table_no_region_fe.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
...
// Make table for other substance use 
	...
	esttab ln_`y'_4  ln_`y'_5  ln_`y'_6 ln_`y'_1 ln_`y'_3  ///
		using "output/tables/other_use_full_table_no_region_fe.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
