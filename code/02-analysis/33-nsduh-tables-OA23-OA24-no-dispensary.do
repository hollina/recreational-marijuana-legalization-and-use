...
foreach y in `y_list' {
	...
	foreach age_group in `age_order_list' {
		...
		reghdfe ln_`y'  i.mm i.rm $list_of_controls if age_category == `age_group',  absorb(StateFIPS year_group_id i.region#i.year_group_id) vce(cluster StateFIPS)
...
// Make table for marijuana use 
	...
	esttab  ln_`y'_4  ln_`y'_5  ln_`y'_6 ln_`y'_1 ln_`y'_3 ///
		using "output/tables/mj_table_no_rm_disp.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
		stats(rm_b_str rm_se_str   ///
			...
			label("\emph{Marijuana use in the past year} &&&&&\\ \addlinespace  \hspace{0.5cm}Recreational legal" "\vspace{.25cm}~" "\hspace{0.5cm}Medical legal"  "\vspace{.25cm}~")) ///
	...
	stats(rm_b_str rm_se_str    ///
		...
		layout("\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}" ///
			 "\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}") ///
		label("\emph{Marijuana use in the past 30 days} &&&&&\\ \addlinespace  \hspace{0.5cm}Recreational legal" "\vspace{.25cm}~" "\hspace{0.5cm}Medical legal"  "\vspace{.25cm}~" )) ///
		drop(*) se b(%9.2f) booktabs ///
		f append  ///
	...
	esttab ln_`y'_4  ln_`y'_5  ln_`y'_6 ln_`y'_1 ln_`y'_3  ///
		using "output/tables/mj_table_no_rm_disp.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
				stats(rm_b_str rm_se_str   mm_b_str mm_se_str   controls state_fe  region_by_year_fe N, fmt( 0 0 0 0 0 0 0 0) ///
		layout("\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}" ///
			 "\multicolumn{1}{c}{@}"  "\multicolumn{1}{c}{@}" ///
			"\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}" ) ///
		label("\emph{Marijuana initiates in past two years} &&&&&\\ \addlinespace  \hspace{0.5cm}Recreational legal" "\vspace{.25cm}~"  "\hspace{0.5cm}Medical legal"  "\vspace{.25cm}~" ///
...
// Make table for other substance use 
	local y alc_use_30 
	esttab ln_`y'_4  ln_`y'_5  ln_`y'_6  ln_`y'_1 ln_`y'_3 ///
		using "output/tables/other_table_no_rm_disp.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
		stats(rm_b_str rm_se_str   ///
			mm_b_str mm_se_str  ///
				, fmt(0 0 0 0) ///
