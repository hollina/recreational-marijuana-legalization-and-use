*! alex_texsave modified texsave (1.5 2nov2020 by Julian Reif) to remove header and footer. 
* added total_frag option which has table fragment similar to esttab output's f option. 
...
program define alex_texsave, nclass
	...
	* Create double-line or thick line, depending on booktabs
	if "`total_frag'" == "" file write `fh' "\toprule" _n
	...
	* SW has a bug with \bottomrule that requires you to output an extra \\
	if "`total_frag'" == ""file write `fh' `"\bottomrule `footnotespace'"' _n(2)	
	...
	* Footnote style #2: just a simple flush left after the end of the table
	...
	if `"`size'"'!="" file write `fh' "}" _n
		...
		if "`sw'"!="" file write `fh' "%BeginExpansion" _n
		if "`total_frag'" == "" file write `fh' "\end{table}" _n
		if "`sw'"!="" file write `fh' "%EndExpansion" _n
