...
replace shape = "1" if shape =="alc_use_30"
replace shape = "2" if shape =="tob_use_30"
replace shape = "3" if shape =="coke_use_365"
destring shape, replace

twoway ///
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 3, msymbol(T) mcolor(black) msize(2.5) ///
	|| rcap   low high order if x!=0 & order<72 & shape==1 & shade == 3,  lcolor(black)  msize(1.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==2 & shade == 3, msymbol(X) mcolor(reddish) msize(2.5) ///
	|| rcap   low high order if x!=0 & order<72 & shape==2 & shade == 3,  lcolor(reddish) msize(1.5)  ///
	|| scatter   x order if x!=0 & order<72 & shape==3 & shade == 3, msymbol(O) mcolor(black) msize(2.5) ///
	|| rcap   low high order if x!=0 & order<72 & shape==3 & shade == 3,  lcolor(black) msize(1.5)  ///	
	|| rcap   low high order if x!=0 & order<72 & shape==2 & shade == 2,  lcolor(reddish%66) msize(1.5)  ///
	...
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 2, msymbol(T) mcolor(white) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 2, msymbol(T) mcolor(black%50) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==2 & shade == 2, msymbol(X) mcolor(white) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==2 & shade == 2, msymbol(X) mcolor(reddish%66) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==3 & shade == 2, msymbol(O) mcolor(white) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==3 & shade == 2, msymbol(O) mcolor(black%50) msize(2.5) ///		
	|| rcap   low high order if x!=0 & order<72 & shape==2 & shade == 1,  lcolor(reddish%33) msize(1.5)  ///
	...
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 1, msymbol(T) mcolor(black) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 1, msymbol(T) mcolor(white) msize(1.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==2 & shade == 1, msymbol(X) mcolor(reddish%33) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==3 & shade == 1, msymbol(O) mcolor(black) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==3 & shade == 1, msymbol(O) mcolor(white) msize(1.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 0, msymbol(Th) mcolor(black) msize(2.5) ///
	|| scatter   x order if x!=0 & order<72 & shape==1 & shade == 0, msymbol(X) mcolor(reddish%33) msize(2.5) ///
	...
	|| scatteri 0 0 0 71, recast(line) lcol(gs10) lp(dash) lwidth(1) /// zero line
		...
		ylabel(-15 "-15" 0 "0" 15 "15"  30 "30") 	///
		aspect(.3) ///
		xsize(9) ///
	 legend( ///
		subtitle("	Alcohol use in past 30 days						Tobacco use in past 30 days			Cocaine use in past 365 days				") ///
		size(4) pos(6) cols(3) order(24 25 26 11 13 15 1 3 5 ) ///
		...
		label(24 "Baseline with state and year fixed-effects") label(25 "Baseline" ) label(26 "Baseline" ) ///
		) 
...
graph export "output/plots/aux_figure_for_paper_robust_part_a.pdf", replace	
graph export "output/plots/aux_figure_for_paper_robust_part_a.png", replace	
