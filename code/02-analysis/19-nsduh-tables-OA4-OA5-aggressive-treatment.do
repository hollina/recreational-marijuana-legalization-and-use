...
foreach y in `y_list' {
	...
	foreach age_group in `age_order_list' {
		...
		reghdfe ln_`y'  i.agr_mm i.agr_rm i.agr_disp i.agr_rm_disp $list_of_controls if age_category == `age_group',  absorb(StateFIPS year_group_id i.region#i.year_group_id) vce(cluster StateFIPS)

		// Store the Recreational legal effect
		//Evaluate Effect using nlcom. Since we will do a tranform of the log results  
		nlcom 100*(exp(_b[1.agr_rm])-1)
		mat b = r(b)
		mat V = r(V)
		...
		//Evaluate Effect using nlcom. Since we will do a tranform of the log results  
		nlcom 100*(exp(_b[1.agr_disp]+_b[1.agr_mm])-1)
		mat b = r(b)
		mat V = r(V)
		...
		//Evaluate Effect using nlcom. Since we will do a tranform of the log results  
		nlcom 100*(exp(_b[1.agr_rm_disp]+_b[1.agr_rm])-1)
		mat b = r(b)
		mat V = r(V)
...
// Make table for marijuana use 
	...
	esttab ln_`y'_4  ln_`y'_5  ln_`y'_6 ln_`y'_1 ln_`y'_3  ///
		using "output/tables/mj_full_table_add_rm_disp_agr.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
...
// Make table for other substance use 
	...
	esttab ln_`y'_4  ln_`y'_5  ln_`y'_6 ln_`y'_1 ln_`y'_3   ///
		using "output/tables/other_use_full_table_add_rm_disp_agr.tex" ///
		,star(* 0.10 ** 0.05 *** .01) ///
