...
capture program drop create_event_study_graph_ln
program create_event_study_graph_ln, 
syntax, ///
	first_time(real) ///
	last_time(real) ///
	...
	marker_size(real)
	/*
	local first_time 2003  
	local 	last_time 2017  
	local 	first_treated_time 2013 
	local 	last_treated_time 2020 
	local 	low_event_cap -6 
	local 	high_event_cap 3 
	local 	low_event_cap_graph -5 
	local 	high_event_cap_graph 2
	local 	y_var y 
	local 	dd_control_list "i.mm i.disp" 
	local 	control_list "ave_median_income ave_unemp_rate ave_male ave_white" 
	local 	fixed_effects "StateFIPS year_numeric" 
	local 	cluster_var "StateFIPS" 
	local 	x_label "Years since treatment" 
	local 	x_lab_step_size 1
	local 	y_label "Y-variable" 
	local 	graph_label "Event study estimates" 
	local 	output_file_name "event-study-program" 
	local 	time_variable year_numeric 
	local 	id_variable StateFIPS 
	local 	weight_var none
	local   marker_color "71 71 179"
	local   marker_symbol "s"
	*/
	////////////////////////////////////////////////////////////////////////////
	// Set up variables
	capture drop weight
	...
	*marker size 
	local out_marker_size = 1 + `marker_size'
	...
	*/
	...
	reghdfe `y_var' ///
		...
		reghdfe y /// 
			_T* ///
			, absorb(StateFIPS year_numeric) cluster(StateFIPS)
		*/
		*http://www.econ.uiuc.edu/~econ472/tutorial6.html
		...
		*quietly {		
			*if `gap' == 1 {
				...
				*matrix list srdvcovbt
				...
				matrix V = get(VCE) 
				matrix list V
				mata st_matrix("tempVCE",select(st_matrix("V"),st_matrix("V")[.,1]:!=0))
				mata st_matrix("VCE",select(st_matrix("tempVCE"),st_matrix("tempVCE")[1,.]:!=0))
				mat list VCE
				

				// Get derivative formula for non-linear transform: 100*(exp(X)-1) = 100*exp(X) - 100 --> dydx = 100*exp(x)
					// set as G
				mata : st_matrix("G", 100*exp(st_matrix("b")))
				mat list G
				
				local max_row = rowsof(G)
				matrix se_store = J(`max_row',1,.)
				di `max_row'
				
				forvalues i = 1(1)`max_row' {
					matrix tempG = J(`max_row',1,0)
					matrix tempG[`i',1] = G[`i',1]
					mat list tempG
					
					matrix GVG=tempG'*VCE*tempG 
					mat list GVG
				
					scalar sepstar=sqrt(el(GVG,1,1)) 
					scalar list sepstar
					*local i = 1
					matrix se_store[`i',1] = sepstar
				}
				
				mata : st_matrix("b_transform", st_matrix("G"):-100)
				mat list b_transform

						
				mata st_matrix("ll", (st_matrix("b_transform")) - 1.96*st_matrix("se_store"))
				mat list ll
				mata st_matrix("ll",select(st_matrix("ll"),st_matrix("ll")[.,1]:!=0))
				mat list ll
				
				mata st_matrix("ul", (st_matrix("b_transform")) + 1.96*st_matrix("se_store"))
				mat list ul
				...
				*local ref_event_time = -1
				...
				mat results =  event_time_names_without_ref, b_transform[1..`max_pos',.], ll[1..`max_pos',.], ul[1..`max_pos',.]
				mat list results
	...
	*/
	...
	twoway rarea low high order if order <= `high_event_cap_graph' & order >= `low_event_cap_graph' , fcol(gs14) lcol(white) msize(3) /// estimates
		|| connected b order if order <= `high_event_cap_graph' & order >= `low_event_cap_graph', lw(1.1) col(white) msize(`out_marker_size') msymbol(`marker_symbol') lp(solid) /// highlighting
		|| connected b order if order <= `high_event_cap_graph' & order >= `low_event_cap_graph', lw(0.6) col("`marker_color'") msize(`marker_size') msymbol(`marker_symbol') lp(solid) /// connect estimates
	...
	*graph export "`output_file_name'.png", replace
	...
	graph save "`output_file_name'.gph", replace
end
